# https://www.alifeintowhitehat.it/post/flask-whooshalchemy-compatibile-con-python-3-6-x/1
...
class _QueryProxy(flask_sqlalchemy.BaseQuery):
    ...
    def __iter__(self):
        ...
        for row in super_rows:
            ...
            if hasattr(row, self._primary_key_name):
                heapq.heappush(
                    ordered_by_whoosh_rank,
                    (self._whoosh_rank[str(getattr(row, self._primary_key_name))], row),
                )
            else:
    ...
    def whoosh_search(self, query, limit=None, fields=None, or_=False):
        ...
        if not results:
            ...
            return self.filter(sqlalchemy.text("null"))  # modified by myself
        ...
        with index.writer() as writer:
            ...
            for update, v in values:
                if update:
                    ...
                    writer.delete_by_term(primary_field, str(getattr(v, primary_field)))
